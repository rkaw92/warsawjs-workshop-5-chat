    ...
    self._app.on('connection', function(socket) {
      ...
      socket.on('auth', function({ login, password }) {
        authenticator.authenticate(login, password).then(function() {
          ...
          const currentLoginHadSocketsBefore = Boolean(self._userSockets.get(login));
          self._unregisterSocket(socket);
          socket[CLIENT_DATA].login = login;
          self._registerSocket(socket);
          ...
          //  new one join.
          if (previousLogin && previousLogin !== login && !self._userSockets.get(previousLogin)) {
            self._notifyUserLeave(previousLogin);
          }
          // If the user had no sockets before, it means he's newly arrived:
          if (!currentLoginHadSocketsBefore) {
            self._notifyUserJoin(socket[CLIENT_DATA].login);
          }
        }, function(error) {
          socket.emit('auth', { success: false, error: error });
        });
      });
